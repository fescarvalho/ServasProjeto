...
// Helper para ajustar prazo e ignorar a conversão automática do navegador
const getAdjustedDeadline = (deadlineString: string) => {
  // 1. Remove o "Z" (UTC) do final da string para forçar horário local
  const cleanDate = deadlineString.endsWith("Z")
    ? deadlineString.slice(0, -1)
    : deadlineString;

  const date = new Date(cleanDate);

  // 2. Ajuste fino para meia-noite
      ...
      <footer className="app-footer no-print">
        <p>
          ...
          <strong>
            {" "}
            <a
              href="https://www.linkedin.com/in/fecarvalhodev/" // <--- COLE SEU LINK AQUI
              target="_blank"
